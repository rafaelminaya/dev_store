...
public interface BoletaVentaRepository extends JpaRepository<BoletaVenta, Long> {
    ...
    List<IReporteRegistroVentasDto> reporteVenta(@Param("fecha_inicio") LocalDateTime fechaInicio, @Param("fecha_fin") LocalDateTime fechaFin);
    ...
    @Query("SELECT b.id AS boletaId, " +
            ...
            "AND b.fechaEmision BETWEEN :fecha_inicio AND :fecha_fin " +
            "GROUP BY p.id")
    List<IReporteLiquidacionDto> findByProductosAgrupadosAntiguo(@Param("productos") List<Long> productosByProveedor, @Param("fecha_inicio") LocalDateTime fechaInicio, @Param("fecha_fin") LocalDateTime fechaFin);
}
